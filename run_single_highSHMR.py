...
while True:
    ...
    params_full = np.loadtxt(inputs.params_dir + 'params_highSHMR.txt')
...
####LF part
    ...
    for index_uv, z_uv in enumerate(lf_zs_saved):
        mturnovers = 10 ** interp1d(np.array(lightcone.node_redshifts)[::-1], np.array(lightcone.log10_mturnovers)[::-1])(
            z_uv
        )
